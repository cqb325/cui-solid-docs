...
:::demo[复杂表单项]{id='customField' src='/form/form/6customField.demo.tsx'}

`FormItem` 只支持绑定第一个表单元素，如果表单元素项下是个复杂多个表单元素，都需要进行校验，有两种方式可以实现，\
1、使用 `FormItem` 组件包裹，并设置 `name` 属性，子表单元素也使用各自的name属性绑定到 `form` 的data属性中。\
除了第一个表单元素，其余的使用 `asFormField` 强制标记为表单元素 `FormItem` 的name值通过 `change` 事件进行赋值。\
2、创建一个自定义表单项组件，在自定义组件中绑定数据到 `form` 中。

:::


:::demo[动态表单]{id='dynamic_form' src='/form/form/7dynamic.demo.tsx'}

form 支持动态表单， formItem的name属性支持索引name，数组为prop.0,prop.1...

:::



:::demo[动态表单2]{id='dynamic_form2' src='/form/form/8dynamic2.demo.tsx'}

form 支持动态表单， formItem的name属性支持索引name，对象为prop.0.xx,prop.1.yy...

:::



:::demo[表格表单]{id='tableForm' src='/form/form/9tableForm.demo.tsx'}
